...
class Dakungall extends CI_Controller
{
    ...
    public function create_action() 
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            notif('0');
            redirect(site_url('dakungall/read/'.$id_crekom.''));
        }
    }
    ...
    {
        $row = $this->Dakungall_model->get_by_idfile($id);
		$id_crkom=$this->session->userdata('id_crekom');
        if ($row) {
            $this->Dakungall_model->del($id);
            redirect(site_url('dakungall/read/'.$id_crkom.''));
        } else {
            redirect(site_url('dakungall/read/'.$id_crkom.''));
        }
    }
