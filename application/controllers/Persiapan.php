...
class Persiapan extends CI_Controller
{
    function __construct()
    {
        parent::__construct();
        is_login();
        $this->load->model('Persiapan_model');
    ...
    {
        $this->template->load('template','persiapan/v_dakungall_list');
    } 
    ...
    public function json() {
        ...
        echo $this->Persiapan_model->json($iduser,$iduserlevel);
    }
    public function filenya($id) {
		$id_crekom=$this->session->userdata('id_crekom');
        header('Content-Type: application/json');
        echo $this->Persiapan_model->jsonfile($id_crekom,$id);
    }
    public function timnya() {
        $id_crkom23=$this->session->userdata('crkom');
        header('Content-Type: application/json');
        echo $this->Persiapan_model->timnya($id_crkom23);
    }
    ...
    {	
        ...
        $row = $this->Persiapan_model->get_by_id($id);
        if ($row) {
            $data = array(
		...
		'id_user' => $row->id_user,
		'button' => 'Unggah',
                'action' => site_url('persiapan/create_action'),
	    );
        $ses_data['crkom']      = $row->id_calon_rekomjak;
		$this->session->set_userdata($ses_data);
            $this->template->load('template','persiapan/v_dakungall_read', $data);
        } else {
            notif('2');
            redirect(site_url('persiapan'));
        }
    }
    ...
    public function create_action() 
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $this->Persiapan_model->insert($data);
            notif('0');
            redirect(site_url('persiapan/read/'.$id_crekom.''));
        }
    }
	function upload_file(){
        $config['upload_path']          = './assets/doc_';
        $config['allowed_types']        = 'jpg|png|pdf|docx|rtf|xlsx|csv|xls|mp4|mp3';
        //$config['max_size']             = 100;
    ...
    //         $this->Persiapan_model->update($this->input->post('', TRUE), $data);
    //         notif('1');
    //         redirect(site_url('dakungall'));
    //     }
    // }
    ...
    public function delete($id) 
    {
        ...
        if ($row) {
            $this->Persiapan_model->delete($id);
            notif('3');
            redirect(site_url('persiapan'));
        } else {
            notif('2');
    ...
    {
        $row = $this->Persiapan_model->get_by_idfile($id);
		$id_crkom=$this->session->userdata('id_crekom');
        if ($row) {
            $this->Persiapan_model->del($id);
            redirect(site_url('persiapan/read/'.$id_crkom.''));
        } else {
            redirect(site_url('persiapan/read/'.$id_crkom.''));
        }
    }
