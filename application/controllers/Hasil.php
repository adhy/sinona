...
class Hasil extends CI_Controller
{
    ...
    {   
        $this->template->load('template','hasil/v_dakungall_list');
    } 
    ...
    public function json() {
        ...
        echo $this->Hasil_model->json($iduser,$iduserlevel);
    }
    public function filenya($id) {
		$id_crekom=$this->session->userdata('id_crekom');
        header('Content-Type: application/json');
        echo $this->Hasil_model->jsonfile($id_crekom,$id);
    }
    public function timnya() {
        $id_crkom23=$this->session->userdata('crkom');
        header('Content-Type: application/json');
        echo $this->Hasil_model->timnya($id_crkom23);
    }
    ...
    {	
		...
		$this->session->set_userdata($ses_data);
            $this->template->load('template','hasil/v_dakungall_read', $data);
        } else {
            notif('2');
    ...
    public function create_action() 
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $data = array(
				...
				//'file_dakung' => $name.'_'.$foto['file_name'],
				'file_dakung' => $foto['file_name'],
	    );

            $this->Hasil_model->insert($data);
            notif('0');
            redirect(site_url('hasil/read/'.$id_crekom.''));
        }
    }
    ...
    //         $this->Hasil_model->update($this->input->post('', TRUE), $data);
    //         notif('1');
    //         redirect(site_url('dakungall'));
    //     }
    // }
    ...
    {
        $row = $this->Hasil_model->get_by_idfile($id);
		$id_crkom=$this->session->userdata('id_crekom');
        if ($row) {
            $this->Hasil_model->del($id);
            redirect(site_url('hasil/read/'.$id_crkom.''));
        } else {
            redirect(site_url('hasil/read/'.$id_crkom.''));
        }
    }
