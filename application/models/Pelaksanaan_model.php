...
class Pelaksanaan_model extends CI_Model
{
    ...
    function json($id,$di) {
        if($di<=3){
            ...
            $this->datatables->add_column('action', anchor(site_url('pelaksanaan/read/$1'),'<i class="fa fa-eye" aria-hidden="true"></i>', array('class' => 'btn btn-success btn-sm')), 'id_calon_rekomjak');
    ...
    function jsonfile($id,$di) {
        ...
        $this->datatables->add_column('action', anchor('#','<i class="fa fa-trash-o" aria-hidden="true"></i>','class="btn btn-danger btn-sm" data-href="'.site_url('pelaksanaan/del/$1').'" data-toggle="modal" data-target="#confirm-delete"'), 'id_dakung');
        return $this->datatables->generate();
    }
    function timnya($id) {
